...
import { Star, ShoppingCart, MapPin, Truck } from 'lucide-react';
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 md:gap-4">
        {products.map((product, index) => {
          ...
          if (products.length === index + 1) {
             return (
               <div ref={lastProductElementRef} key={`${product.id}-${index}`} className="h-full">
                 <Link
                    ...
                    <div className="p-3 sm:p-4 flex flex-col flex-1">
                      ...
                      <div className="mt-auto flex items-center justify-between">
                        <div className="flex flex-col">
                          <span className="text-base sm:text-xl font-bold text-primary-600">{product.price}</span>
                          {product.originalPrice && (
                            <span className="text-[10px] sm:text-xs text-gray-400 line-through">{product.originalPrice}</span>
                          )}
                        </div>
                        <button className="w-8 h-8 sm:w-10 sm:h-10 bg-primary-500 text-white rounded-full flex items-center justify-center hover:bg-primary-600 transition-all shadow-green-sm">
